...
def index(request):
    ...
    responses = UserResponse.objects.filter(user=user).select_related('question').prefetch_related('selected_choices')
...
def take_assessment(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            for question in Question.objects.prefetch_related('choices'):
                response = form.save_response(request.user, question)
                if response:
                    responses.append(response)
                    total_score += response.total_score()
            
            normalized_score = round(min(100, (total_score / max_possible_score) * 100), 1) if max_possible_score > 0 else 0
            ...
            AssessmentHistory.objects.create(
                user=request.user,
                score=normalized_score,
                result_text=get_result_text(normalized_score),
            )
            
            # Limit history records to 20
            history_count = AssessmentHistory.objects.filter(user=request.user).count()
            if history_count > 20:
                oldest_records = AssessmentHistory.objects.filter(user=request.user).order_by('date_taken')[:history_count-20]
                AssessmentHistory.objects.filter(id__in=oldest_records.values_list('id', flat=True)).delete()
    ...
    return render(request, 'assessment/take_assessment.html', {'questions': questions, 'form': form})

@login_required
def assessment_result(request):
    responses = UserResponse.objects.filter(user=request.user).select_related('question').prefetch_related('selected_choices').order_by('-submitted_at')
    total_score = sum(response.total_score() for response in responses)
    normalized_score = request.session.get('assessment_score', 0)
    result = get_result(normalized_score)
...
def generate_certificate(request):
    user = request.user
    latest_assessment = get_object_or_404(AssessmentHistory, user=user, certificate__isnull=True)
    
    normalized_score = latest_assessment.score
    result = get_result(normalized_score)
    
    pdf_content = generate_certificate_pdf(user, normalized_score, result)
    
    # Save to AssessmentHistory
    certificate_filename = f'certificate_{user.username}_{latest_assessment.id}.pdf'
    latest_assessment.certificate.save(certificate_filename, ContentFile(pdf_content), save=True)
    ...
    response['X-Sendfile'] = certificate_filename
...
@login_required
def assessment_history(request):
    history = AssessmentHistory.objects.filter(user=request.user).order_by('-date_taken')
    context = {
        'history': history,
    }
    return render(request, 'assessment/assessment_history.html', context)